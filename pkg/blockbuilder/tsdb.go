...
func (b *TSDBBuilder) newTSDB(tenant tsdbTenant) (*userTSDB, error) {
	...
	db, err := tsdb.Open(udir, util_log.SlogFromGoKit(userLogger), tsdbPromReg, &tsdb.Options{
		...
		EnableNativeMetadata:                 b.limits.OTelPersistResourceAttributes(userID),
		EnableResourceAttrIndex:              b.limits.OTelResourceAttrIndexEnabled(userID),
		IndexedResourceAttrs:                 stringSliceToSet(b.limits.OTelIndexedResourceAttributes(userID)),
	}, nil)
	if err != nil {
		return nil, err
	}
