...
// updateIndexedResourceAttrs reconfigures the indexed resource attributes for
// each tenant's TSDB head when the per-tenant limit changes at runtime.
func (i *Ingester) updateIndexedResourceAttrs() {
	for _, userID := range i.getTSDBUsers() {
		userDB := i.getTSDB(userID)
		if userDB == nil {
			continue
		}
		existing := userDB.Head().GetIndexedResourceAttrs()
		if existing == nil {
			// Series metadata is disabled for this tenant; SetIndexedResourceAttrs
			// would be a no-op, so skip the allocation.
			continue
		}
		newSlice := i.limits.OTelIndexedResourceAttributes(userID)
		if sliceEqualsSet(newSlice, existing) {
			continue
		}
		userDB.Head().SetIndexedResourceAttrs(stringSliceToSet(newSlice))
	}
}
...
func sliceEqualsSet(slice []string, set map[string]struct{}) bool {
	...
	// If the unique count matches, set⊆slice is implied.
...
func (i *Ingester) createTSDB(userID string, walReplayConcurrency int) (*userTSDB, error) {
	...
	db, err := tsdb.Open(udir, util_log.SlogFromGoKit(userLogger), tsdbPromReg, &tsdb.Options{
		...
		EnableNativeMetadata:                 i.limits.OTelPersistResourceAttributes(userID),
		EnableResourceAttrIndex:              i.limits.OTelResourceAttrIndexEnabled(userID),
		IndexedResourceAttrs:                 stringSliceToSet(i.limits.OTelIndexedResourceAttributes(userID)),
